...
try:
        solver = Solver(
            ...
            "orientationMatch": OrientationMatchObjective(
                name="MyOrientationMatchObjective", link="F3_TIP", weight=10.0
            ),
...
for i in range(10000):    
    ...
    # state = solver.solve(goals= {"positionMatch": Translation(x=0.5, y =0.5, z=0.5), "jointMatch1": 1.5, "jointMatch2": 1.5, "jointMatch3": 1.5},weights = {},time = 0.0)
    joint_states_list = []
    sorted_keys = sorted(state.joints, key=lambda x: desired_order.index(x))
    joint_states_list = [state.joints[k] for k in sorted_keys]
    # p.setJointMotorControlArray(
    ...
    p.resetBasePositionAndOrientation(
        sphere_id_2, 
        [temp2[0], temp2[1], temp2[2]],  # Position of the first fingertip
        [0, 0, 0, 1]  # Quaternion for orientation (no rotation)
    )
    p.resetBasePositionAndOrientation(
        sphere_id_3, 
        [temp3[0], temp3[1], temp3[2]],  # Position of the first fingertip
        [0, 0, 0, 1]  # Quaternion for orientation (no rotation)
    )
